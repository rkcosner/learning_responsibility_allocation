...
from tbsim.datasets.avdata_datamodules import UnifiedDataModule
...
def datamodule_factory(cls_name: str, config):
    ...
    if cls_name.startswith("L5"):  # TODO: make this less hacky
        ...
        avdata_config = translate_avdata_cfg(config)
        datamodule = eval(cls_name)(data_config=avdata_config, train_config=config.train)
    else:
