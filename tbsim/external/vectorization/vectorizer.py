...
class Vectorizer:
    ...
    def __init__(self, cfg: dict, mapAPI: MapAPI):
        ...
        if "vectorize_map" in cfg["model_params"].keys():
            self.vectorize_map = cfg["model_params"]["vectorize_map"]
        else:
            self.vectorize_map = True
        self.mapAPI = mapAPI
        self.max_agents_distance = cfg["data_generation_params"]["max_agents_distance"]
        self.history_num_frames_agents = cfg["model_params"][
            "history_num_frames_agents"
        ]
        self.future_num_frames = cfg["model_params"]["future_num_frames"]
        self.history_num_frames_max = max(
            cfg["model_params"]["history_num_frames_ego"],
            self.history_num_frames_agents,
        )
    ...
    def vectorize(
        ...
        history_yaws_rad: np.ndarray,
        history_availability: np.ndarray,
        ...
        """
        agent_features = self._vectorize_agents(
            selected_track_id,
            agent_centroid_m,
            agent_yaw_rad,
            agent_from_world,
        ...
        if self.vectorize_map:
            map_features = self._vectorize_map(
                agent_centroid_m, agent_from_world, history_tl_faces
            )
        else:
            map_features = {}
        return {**agent_features, **map_features}

    def _vectorize_agents(
        self,
        selected_track_id: Optional[int],
        agent_centroid_m: np.ndarray,
        agent_yaw_rad: float,
        ...
        history_agents_flat = filter_agents_by_distance(
            history_agents_flat, agent_centroid_m, self.max_agents_distance
        )
        ...
        list_agents_to_take = get_other_agents_ids(
            history_agents_flat["track_id"],
            cur_agents["track_id"],
            selected_track_id,
            self.other_agents_num,
        )

        # Loop to grab history and future for all other agents
        all_other_agents_history_positions = np.zeros(
            (self.other_agents_num, self.history_num_frames_max + 1, 2),
            dtype=np.float32,
        )
        all_other_agents_history_yaws = np.zeros(
            (self.other_agents_num, self.history_num_frames_max + 1, 1),
            dtype=np.float32,
        )
        ...
        all_other_agents_future_yaws = np.zeros(
            (self.other_agents_num, self.future_num_frames, 1), dtype=np.float32
        )
        all_other_agents_future_extents = np.zeros(
            (self.other_agents_num, self.future_num_frames, 2), dtype=np.float32
        )
        ...
        for idx, track_id in enumerate(list_agents_to_take):
            ...
            current_other_actor = filter_agents_by_track_id(
                history_agents_flat, track_id
            )[0]
            all_other_agents_types[idx] = np.argmax(
                current_other_actor["label_probabilities"]
            )
        ...
        all_other_agents_history_availability[
            :, self.history_num_frames_agents + 1 :
        ] *= 0
        ...
        other_agents_polyline_availability = (
            all_other_agents_history_availability.copy()
        )
        ...
        agent_dict = {
            ...
            "all_other_agents_history_availability": all_other_agents_history_availability.astype(
                np.bool
            ),
            ...
            "all_other_agents_future_availability": all_other_agents_future_availability.astype(
                np.bool
            ),
            ...
            "other_agents_polyline_availability": other_agents_polyline_availability.astype(
                np.bool
            ),
    ...
    def _vectorize_map(
        ...
        lanes_availabilities = np.zeros(
            (MAX_LANES * 2, MAX_POINTS_LANES), dtype=np.float32
        )

        lanes_mid_points = np.zeros((MAX_LANES, MAX_POINTS_LANES, 2), dtype=np.float32)
        lanes_mid_availabilities = np.zeros(
            (MAX_LANES, MAX_POINTS_LANES), dtype=np.float32
        )
        ...
        lanes_indices = indices_in_bounds(
            agent_centroid_m, lanes_bounds, MAX_LANE_DISTANCE
        )
        distances = []
        for lane_idx in lanes_indices:
            lane_id = self.mapAPI.bounds_info["lanes"]["ids"][lane_idx]
            lane = self.mapAPI.get_lane_as_interpolation(
                lane_id, STEP_INTERPOLATION, INTERP_METHOD
            )
            lane_dist = np.linalg.norm(
                lane["xyz_midlane"][:, :2] - agent_centroid_m, axis=-1
            )
        ...
        crosswalks_indices = indices_in_bounds(
            agent_centroid_m, crosswalks_bounds, MAX_LANE_DISTANCE
        )
        crosswalks_points = np.zeros(
            (MAX_CROSSWALKS, MAX_POINTS_CW, 2), dtype=np.float32
        )
        ...
        active_tl_faces = set(
            filter_tl_faces_by_status(history_tl_faces[0], "ACTIVE")["face_id"].tolist()
        )
        active_tl_face_to_color: Dict[str, str] = {}
        for face in active_tl_faces:
            try:
                active_tl_face_to_color[face] = self.mapAPI.get_color_for_face(
                    face
                ).lower()  # TODO: why lower()?
            except KeyError:
        ...
        for out_idx, lane_idx in enumerate(lanes_indices[:MAX_LANES]):
            ...
            lanes_tl_feature[
                out_idx, :num_vectors_mid
            ] = self.mapAPI.get_tl_feature_for_lane(lane_id, active_tl_face_to_color)
        ...
        valid_mid_distances = (
            np.linalg.norm(lanes_mid_points, axis=-1) < MAX_LANE_DISTANCE
        )
        ...
        lanes = np.concatenate(
            [lanes_points, np.zeros_like(lanes_points[..., [0]])], axis=-1
        )
        # pad such that length is 3
        crosswalks = np.concatenate(
            [crosswalks_points, np.zeros_like(crosswalks_points[..., [0]])], axis=-1
        )
