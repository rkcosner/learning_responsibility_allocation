...
from l5kit.l5kit.rasterizer import (
    EGO_EXTENT_HEIGHT,
    EGO_EXTENT_LENGTH,
    EGO_EXTENT_WIDTH,
    Rasterizer,
...
def draw_boxes(
    ...
    box_raster_coords = transform_points(
        box_world_coords.reshape((-1, 2)), raster_from_world
    )
        ...
        for i, (frame, agents) in enumerate(zip(history_frames, history_agents)):
            ...
            if agent is None:
                ...
                if len(ego_agent) > 0:  # check if ego_agent is in the frame
                    agents = agents[
                        agents != ego_agent[0]
                    ]  # remove ego_agent from agents
            agents_images[..., i] = draw_boxes(
                self.raster_size, raster_from_world, agents, 255
            )
            if len(ego_agent) > 0 and (self.render_ego_history or i == 0):
                ego_images[..., i] = draw_boxes(
                    self.raster_size, raster_from_world, ego_agent, 255
                )
