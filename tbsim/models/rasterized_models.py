...
class RasterizedDiscreteVAEModel(nn.Module):
    ...
    def sample(self, batch_inputs: dict, n: int,cond_traj = None):
        ...
        if self.dyn is not None:
            curr_states = batch_utils().get_current_states(batch_inputs, self.dyn.type())
            decoder_kwargs["current_states"] = curr_states

        outs = self.vae.sample(condition_inputs=condition_inputs, n=n, decoder_kwargs=decoder_kwargs)
        if self.EC:
            outs = TensorUtils.join_dimensions(outs,0,2)
    ...
    def compute_losses(self, pred_batch, data_batch):
        ...
        pred_loss = compute_pred_loss(self.recon_loss_type,pred_batch,target_traj,data_batch["target_availabilities"],prob,self.weights_scaling)
        ...
        if "EC_pred" in pred_batch:
            ...
            deviation_loss = compute_pred_loss(self.recon_loss_type,EC_batch,target_traj_tiled,avails,EC_prob,self.weights_scaling)
        ...
        for t in range(self.stage):
            ...
            if t==0:
                ...
                if self.EC:
                    ...
                    current_bs = np.prod(batch_shape)
            ...
            if not sample:
                ...
                inputs=None
            if self.EC:
                cond_traj_tiled = cond_traj[t].repeat_interleave(int(np.prod(batch_shape[1:])),0)
