...
def plot_gammas(batch, net, relspeed=0.0, B=0, A=0):
    ...
    cs = ax_cont.contour(gridX.numpy(), gridY.numpy(), np.flip(np.flip(gammasA, axis=0),axis=1, colors='k', linewidths=3, linestyles="solid")
    ax_cont.contour(gridX.numpy(), gridY.numpy(), np.flip(np.flip(gammasA, axis=0),axis=1, cmap=plt.cm.coolwarm, linewidths=2)
    ax_cont.clabel(cs, cs.levels[np.where(cs.levels!=0)])
        # Plot 0 level 
    cs = ax_cont.contour(gridX.numpy(), gridY.numpy(), np.flip(np.flip(gammasA, axis=0),axis=1, levels=0, colors='k', linewidths=5)
    ax_cont.contour(gridX.numpy(), gridY.numpy(), np.flip(np.flip(gammasA, axis=0),axis=1, levels=0, colors='w', linewidths=3, linestyles='--')
